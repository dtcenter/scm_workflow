...
class Experiment(object):
    ...
    def setup_rundir(self):
        ...
        # Copy physics namelist to run directory with its original name
        logging.debug('Copying physics namelist {0} to run directory'.format(self._physics_namelist))
        ...
        if not os.path.isfile(os.path.join(SCM_ROOT, PHYSICS_NAMELIST_DIR, self._physics_namelist)):
            ...
            logging.critical(message)
            raise Exception(message)
        cmd = "cp {0} {1}".format(os.path.join(SCM_ROOT, PHYSICS_NAMELIST_DIR, self._physics_namelist), os.path.join(SCM_RUN, self._physics_namelist))
        if surface_flux_spec:
            ...
            if os.path.isfile(opt_ps_nml_filename):
                ...
                cmd = "cp {0} {1}".format(opt_ps_nml_filename, os.path.join(SCM_RUN, self._physics_namelist))
        execute(cmd)
